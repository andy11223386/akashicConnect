...
router.post('/getMyRetweet/:username', async (req, res) => {
  try {
    const { username } = req.params;
    ...
    // Find all tweets where the retweets array contains the username
    const retweetedTweets = await Tweet.find({ retweets: username }).sort({ createdAt: -1 });

    // Prepare the retweeted tweets with additional user details
    const tweetsWithDetails = await Promise.all(retweetedTweets.map(async (tweet) => {
      const user = await User.findOne({ username: tweet.username });
    ...
    if (tweetsWithDetails.length === 0) {
      return res.status(404).send({ message: 'No retweets found for this user' });
    }
